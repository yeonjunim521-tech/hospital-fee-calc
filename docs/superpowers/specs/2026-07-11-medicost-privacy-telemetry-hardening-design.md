...
> 2026-08-30 변경: `원시 검색어를 저장하지 않는다`는 검색 telemetry 결정은 최신 사용자 결정으로 대체됐다. 현재 계약은 `docs/decisions/2026-08-30-search-analytics-contract.md`를 따른다. 개인 식별정보 비연결, 외부 분석 최소화, 30일 보존, 철회 후 중단 원칙은 유지한다.
