...
- 선택 파라미터:
  - `page_path`: 현재 페이지 경로
- 목적: 검색 실행량과 결과 제공 여부를 본다. 실제 검색어는 GA4가 아니라 자체 D1 로그에서만 30일간 확인한다
...
- 선택 파라미터:
  - `page_path`: 현재 페이지 경로
- 목적: 무결과 검색 발생량을 본다. 구체적인 무결과 검색어는 자체 D1 로그에서 확인한다
...
- 선택 파라미터:
  - `page_path`: 현재 페이지 경로
- 목적: 검색 결과 클릭량을 본다. 검색어와 클릭 항목은 자체 D1 로그에서 확인한다
...
- 필수 파라미터:
  ...
  - `final_cost_bucket`: 최종 환자 부담금 구간
- 선택 파라미터:
  - `page_path`: 현재 페이지 경로
...
- 숫자 값(`result_count`)은 문자열이 아니라 숫자로 보내는 현재 방식이 적절하다.
- GA4에는 원시 검색어, 의료 항목 코드·이름, 정확한 금액을 보내지 않는다.
