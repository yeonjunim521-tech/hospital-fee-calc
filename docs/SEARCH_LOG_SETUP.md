...
오늘, 24시간, 최근 3일, 7일, 30일, 90일 통계를 볼 수 있다.

관리자 페이지는 `robots.txt`와 `noindex`로 검색 노출을 막는다. 단, 주소를 아는 사람은 조회할 수 있으므로 공개 링크로 노출하지 않는다.

## 주의

- 주민등록번호, 전화번호, 이메일 형태가 검색어·항목 코드·항목명·경로 중 하나에라도 있으면 API에서 저장하지 않는다.
- 검색어, 정규화 검색어, 결과 수, 검색 경로는 분석 동의가 있을 때만 30일간 저장한다.
- 클릭 로그는 검색어, 클릭한 의료 항목 코드·이름, 검색 경로를 30일간 저장한다.
- IP는 시간당 요청 제한을 계산할 때만 해시 처리하며 검색·클릭 로그에는 저장하지 않는다. User-Agent도 저장하지 않는다.
- 과거 최소수집 기간에 기록된 `aggregate` 행은 관리자 검색·클릭 통계에서 제외한다.
...
- 직접선택 드롭다운과 분류 목록 선택은 검색 통계에 포함하지 않는다.
- 공개 telemetry payload는 형식 검증과 IP별 시간당 제한을 거치지만 클라이언트 입력이므로, 운영 통계는 참고 지표로 취급한다.
